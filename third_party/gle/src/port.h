...
/* PATCH(xss-sdl): this tree has no system OpenGL -- desktop GL comes
 * from gl4es (and GLU from the vendored glues), on macOS as much as
 * anywhere else, so take the generic <GL/...> branch on every
 * platform. Without this, Darwin would pull in Apple's deprecated
 * framework headers and the hack would link against two different GLs. */
#include <GL/gl.h>
#include <GL/glu.h>
