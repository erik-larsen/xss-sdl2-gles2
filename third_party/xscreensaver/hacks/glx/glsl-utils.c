                                    ...
                                    GLboolean *gl_gles3)
{
  ...
  /* PATCH(xss-sdl): kill switch. The SDL port runs GLSL-capable hacks
     on their fixed-function fallbacks (translated by gl4es); the
     GLSL-through-gl4es path is a future experiment. Default set in
     src/port/glx-sdl.c; unset XSS_DISABLE_GLSL to re-enable. */
  if (getenv ("XSS_DISABLE_GLSL"))
    return GL_FALSE;
