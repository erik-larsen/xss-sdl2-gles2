...
/* PATCH(xss-sdl): darwin ld has no __attribute__((alias)); forward
 * explicitly. Real definitions are required -- gl_lookup.c references
 * both symbols. (Upstream gap: AliasExport is __APPLE__-guarded,
 * AliasDecl is not.) */
#ifdef __APPLE__
...
/* PATCH(xss-sdl): see gl4es_glEnableClientStatei above. */
#ifdef __APPLE__
