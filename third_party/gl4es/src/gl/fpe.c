    ...
    if(glprogram->va_size[i])   // only check used VA...
    {
        ...
        if(v->enabled) {
            ...
            if(dirty || v->size!=w->size || v->type!=w->type || v->normalized!=w->normalized 
                ...
                /* PATCH(xss-sdl): WebGL forbids client-side vertex arrays.
                   When real_buffer==0 the attrib data is a client (heap)
                   pointer, which emscripten's FULL_ES2 emulation will upload
                   for us -- but only if it sees NO ARRAY_BUFFER bound at
                   glVertexAttribPointer time. gl4es's bindBuffer() caches the
                   binding and skips the GLES call when it believes nothing is
                   bound; meanwhile emscripten's own FULL_ES2 path can leave its
                   temp vertex buffer actually-bound after an earlier client
                   draw (e.g. the framebuffer blit). The result is a cache
                   desync: gl4es thinks ARRAY_BUFFER==0 while WebGL still holds a
                   stale buffer, so the client pointer is misread as an offset
                   into it -> every draw produces zero fragments (all GL hacks
                   render opaque black). Force a real unbind here, bypassing the
                   cache, so emscripten reliably treats this as a client array.
                   The buffered path (real_buffer!=0) is unchanged. */
                if (v->real_buffer == 0) {
