...
## M10 — Bundled CC0 image set for the grab hacks

- **M10a ✅ real photos instead of colour bars.** assets/images/: 11
  CC0 landscape photos (~2.5MB total; coast, lake, canyon, dunes,
  waterfall, winter, river, wave, clouds, autumn, hills), sourced via
  the Openverse API with license=cc0 filtering; per-image provenance
  (title/creator/source/landing URL) recorded in assets/images/
  README.md. Resized to <=1600px, JPEG q82.
  Plumbing: grabclient-sdl.c's jwxyz_draw_random_image() now decodes a
  random image with stb_image (vendored third_party/stb_image.h,
  public domain, JPEG+PNG only) and scale-to-cover bilinear-blits it
  into the drawable as a jwxyz XImage (same RGBA order as
  jwxyz_png_to_ximage). Sources: native reads $XSS_IMAGE_DIR or
  ./assets/images (POSIX dirent; Windows returns NULL until that port
  lands); web prefetches ONE random image in the shell before the
  runtime starts (preRun + addRunDependency; ../images/index.json ->
  ../images/<pick>, shared across all hack pages, generated by
  deploy-web.sh) and the hook copies the bytes out of Module
  synchronously. Any failure (no dir, offline, fetch error) returns
  NULL -> grabclient's colour-bars fallback, so nothing regresses.
  Verified native (decayscreen melts the Big Sur coast; antspotlight's
  ant spotlights it) and web (decayscreen 99.7% coverage/6.5k colors,
...
- **M13c ✅ vigilance renders: gl4es fog-in-display-list replay bug.** Wireframe rendered but fill mode was solid black, which pointed at the fill-only state: fog. vigilance compiles its `glFog*` setup (MODE, COLOR, DENSITY, START) into the GROUND display list; gl4es records each call in its own list segment (`rlFogOp`, one op per segment, `StageExclusive[STAGE_FOG]=1`), but the replay in listdraw.c ignored the recorded pname and executed *every* fog call as `GL_FOG_COLOR`. So mode and density stayed at their defaults (GL_EXP, density 1.0) — `exp(-fog_c)` ≈ 0 at scene distances — and the whole scene fogged to solid black. One-line `PATCH(xss-sdl)` in third_party/gl4es/src/gl/listdraw.c: replay `gl4es_glFogfv(list->fog_op, list->fog_val)`. Verified native (cameras + green grid, maxLum 203) and web (verify-web.js nonBlank, delta 0.0245). The other fog hacks all pass because they issue fog calls outside display lists; any hack compiling fog into a dlist was exposed.

- **M13d ✅ peepers renders on web: glBegin-recorded display lists drew nothing under FULL_ES2.** The eyeballs were invisible on the web (only the untextured retina's dark-red dot showed, maxLum 17); native was fine. Instrumenting the page (puppeteer hooks on texImage2D / vertex attribs / bufferSubData) walked every layer: texture data uploaded correctly (512×512 RGBA, sane pixels), texcoord attrib bound, sampler on the right unit, lighting uniforms bright, no GL errors — and then the actual defect: **every vertex-attrib upload before the eyeball's `drawElements` was 0 bytes**. Root cause is an interaction of three pieces:
  1. `gl4es_glBegin` stamps its renderlist `use_vbo_array = 2` (a "small optim"), so a *compiled* display list whose geometry came through glBegin/glEnd (peepers' `draw_ball`) never gets `list2VBO`'d — its attribs replay as client-side pointers.
  2. The replay still creates a real ELEMENT_ARRAY_BUFFER for the indices.
  3. emscripten's FULL_ES2 `glDrawElements` computes the client-attrib upload size by scanning *client-side* indices; with an element VBO bound it leaves `vertexes = 0` and uploads zero bytes per attrib. Draw executes, no error, zero fragments.
  `PATCH(xss-sdl)` in third_party/gl4es/src/gl/listdraw.c, emscripten-only: when a list's attribs are client-side (no list VBO), keep the indices client-side too, so emscripten scans them and sizes the uploads correctly. Lists whose attribs did get a real VBO keep their element VBO. Verified: peepers web now draws full textured lit eyeballs (nonBlackFrac 0.23, 1167 colors vs 3 before); vigilance web re-checked (camera models + grid). Native untouched. Any other hack whose web rendering was missing glBegin-compiled indexed geometry should improve in the next sweep.

- **M13e ✅ dymaxionmap was never slow — the harness's `--frames` was being eaten by the hack.** `consume_hack_options` folds GNU `--opt` into the X-style `-opt` a hack table declares, and dymaxionmap declares its own `-frames` (day/night animation frame count, default 720). So `--frames 3` set that resource instead of the driver's frame limit; the driver ran unbounded (`frames_limit=0`, confirmed with lldb: `frames_done=9631` and climbing at ~50fps) and the harness's 10-minute cap read as "worse than 20 s/frame". With the fix, 30 frames + shot take 1.5 s and the shot shows the unfolded map, terminator and stars. Fix in screenhack-sdl.c: the driver's flags (`--width --height --frames --shot --fps`) are reserved in their double-dash spelling and pass through before GNU folding; single-dash still reaches the hack. Also rescued: anemone (`-width`), rdbomb (`-width -height`), splitflap (`-width -height`) — those hack options are counts, not pixels, and had been swallowing the harness's window geometry. Lesson for the gotchas list: a hack that "times out" under the harness may just not be receiving `--frames`.
